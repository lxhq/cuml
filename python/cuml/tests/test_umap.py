...
@pytest.mark.parametrize("kind", ["csr", "csc", "coo", "pairwise", "tuple"])
def test_extract_knn_graph(in_mem_type, in_dtype, kind):
    ...
    if kind == "pairwise":
        ...
        knn_info = kneighbors_graph(X, 20, mode="distance")
        if in_mem_type == "device":
        ...
        knn_info.sum_duplicates()

    sol = kneighbors_graph(X, 10, mode="distance")

    # mem_type=None doesn't coerce mem_type
    indices, distances = extract_knn_graph(knn_info, 10, mem_type=None)
    xp = cp if in_mem_type == "device" else np
    assert isinstance(indices, xp.ndarray)
    assert isinstance(distances, xp.ndarray)
    np.testing.assert_allclose(cp.asnumpy(distances), sol.data)
    np.testing.assert_array_equal(cp.asnumpy(indices), sol.indices)

    indices, distances = extract_knn_graph(knn_info, 10, mem_type="host")
    np.testing.assert_allclose(distances, sol.data)
    np.testing.assert_array_equal(indices, sol.indices)

    indices, distances = extract_knn_graph(knn_info, 10, mem_type="device")
    np.testing.assert_allclose(distances.get(), sol.data)
...
def test_extract_knn_graph_errors():
    ...
    knn_info = sp.csr_matrix(
        (np.array([0.5, 0.5, 0.5]), (np.array([0, 0, 1]), np.array([0, 1, 0])))
    )
    with pytest.raises(
        ValueError, match="Precomputed KNN graph has 3 total elements"
    ):
        extract_knn_graph(knn_info, 2)
...
def test_extract_knn_graph_from_kneighbors_graph_zero_copy(kind, mem_type):
    ...
    with cuml.using_output_type(kind):
        graph = cuml.neighbors.kneighbors_graph(X, 20, mode="distance")
